...
#DRAFT
def expands(item, tokens, visited):
    parts = []
    parts.append(item)
    print("7 ::", parts)
    ...
    if hasattr(parts[-1], 'rights'):
        for item2 in parts[-1].rights:
            print("7.5 ::", item2, item2.pos_, item2.dep_)
            if item2.pos_ == "DET" or item2.pos_ == "NOUN": 
                print("7.6 item2.i ::", item2.i)
                if item2.i not in visited:  
                    visited.add(item2.i)  
                    print("8 ::", visited)  
                    print("9 :", expand(item2, tokens, visited))
...
def to_str(tokens):
    if isinstance(tokens, Iterable):
        print("->", [item.text for item in tokens])
                ...
                for sub in subs:
                    if len(objs) > 0:
                        for obj in objs:
                            objNegated = _is_negated(obj)
                            if is_pas:  # reverse object / subject for passive
                                print("word-1 :",sub, "|", obj, "|", tokens, "|", visited)
                                svos.append((to_str(expand(obj, tokens, visited)),
                                             "!" + v.lemma_ if verbNegated or objNegated else v.lemma_, to_str(expand(sub, tokens, visited))))
                            else:
                                print("word-2 :",sub, "|", obj, "|", tokens, "|", visited)
                                svos.append((to_str(expand(sub, tokens, visited)),
                                             "!" + v.lower_ if verbNegated or objNegated else v.lower_, to_str(expand(obj, tokens, visited))))
                                print("5.2.1:",svos)                 
                    else:
                        # no obj - just return the SV parts
                        print("word-3 :",sub, "|", tokens, "|", visited)
                        svos.append((to_str(expand(sub, tokens, visited)),
